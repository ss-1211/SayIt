...
class UserPostCell: UITableViewCell {
    ...
    @IBOutlet weak var postConstraint: NSLayoutConstraint!
    ...
    override func awakeFromNib() {
        ...
        userPost.lineBreakMode = .byWordWrapping
        let frame = CGSize(width: self.frame.width - 8, height: CGFloat.greatestFiniteMagnitude)
        var rect = userPost.sizeThatFits(frame)
        if(rect.width<250){
            rect.width=30
        }
        postConstraint.constant = rect.width//テキストが短くても最小のビューの幅を30とする
    }
...
//        userPost.isUserInteractionEnabled = true
    ...
    //セルへ配置
    func setUp(userIcon: UIImage, userName: String, userPost: String, userPostImage: UIImage?, adminPost: String?){
        self.userIcon.image = userIcon
        self.userName.text = userName
        self.userPost.text = userPost
        if userPostImage == nil {
            self.userPostImage.isHidden = true
        }else{
            self.userPostImage.image = userPostImage
        }
        if adminPost == "" {
            self.adminIcon.isHidden = true
            self.adminPost.isHidden = true
        }else{
